...
LOG_FILE = "submissions/latency.txt"
...
def main():
    ...
    if not texts:
        log("No texts found in input file.")
        return
    ...
    log(f"Latency over {args.runs} runs (batch_size=1):")
    log(f"  p50: {p50:.2f} ms")
    log(f"  p95: {p95:.2f} ms")

if __name__ == "__main__":
    # Ensuring results.txt exists before logging
    if not os.path.exists(LOG_FILE):
        with open(LOG_FILE, "w", encoding="utf-8") as f:
            f.write("Latency for 50 runs:\n")
    main()
